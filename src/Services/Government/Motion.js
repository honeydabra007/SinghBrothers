...
const ProgressBar = () => {
    ...
    useEffect(() => {
        const marketResearchInterval = setInterval(() => {
            setMarketResearchProgress(prevProgress => {
                ...
                if (newProgress >= 80) clearInterval(marketResearchInterval); // Stop when reaching 85%
                return newProgress;
            });
        }, 25);
        ...
        const teamLeadingInterval = setInterval(() => {
            setTeamLeadingProgress(prevProgress => {
                ...
                if (newProgress >= 70) clearInterval(teamLeadingInterval); // Stop when reaching 75%
                return newProgress;
            });
        }, 30);
    ...
    return (
        <div className='flex flex-col gap-6 h-screen p-40 w-full'>
